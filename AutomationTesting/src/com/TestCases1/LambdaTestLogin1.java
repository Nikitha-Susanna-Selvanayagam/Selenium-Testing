...
public class LambdaTestLogin1 {
	
	//Created reference of WebDriver
	public static WebDriver ok;
	...
	public void setUp()
	{
		...
		//Added implicit wait
		ok.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
		
		//Added to wait till page gets loaded completely
		ok.manage().timeouts().pageLoadTimeout(15, TimeUnit.SECONDS);
	...
	public void login()
	{
		...
		ok.get("https://www.lambdatest.com/");
		
		//Identify and Click on Login link present on right top side
		WebElement logInLink = ok.findElement(By.xpath("//a[contains(text(),'Log in')]"));
		logInLink.click();
		
		//On login page, identify email id field and enter registered email id
		WebElement emailField = ok.findElement(By.xpath("//input[@placeholder='Email']"));
		emailField.sendKeys("username@gmail.com");
		
		//On login page, identify user password field and enter valid password
		WebElement passwordField = ok.findElement(By.xpath("//input[@placeholder='Password']"));
		passwordField.sendKeys("Test@123");
		
		//On login page, identify login button and click it
		WebElement loginButton = ok.findElement(By.xpath("//button[@class='btn btn-primary btn-lg btn-block mt-3']"));
		loginButton.click();
		...
		String actualURL = ok.getCurrentUrl();
		String actualPageTitle = ok.getTitle();
		System.out.println(ok.getTitle());
		System.out.println(ok.getCurrentUrl());
